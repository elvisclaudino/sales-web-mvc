...
namespace SalesWebMvc.Data
{
    public class SeedingService
    {
        ...
        public void Seed()
        {
            ...
            {
                Console.WriteLine($"Erro ao salvar as alterações no banco de dados: {e.Message}");

                // Verifica se há uma exceção interna para obter detalhes adicionais
                if (e.InnerException != null)
                {
                    Console.WriteLine($"Detalhes da exceção interna: {e.InnerException.Message}");
                }

                throw; // Rethrow a exceção para permitir a análise e correção
            }
        }
    }
}
